...
{
    public class AnomalyTrainViewModel : ViewModelBase
    {
        ...
        /// </summary>
        /// 
        private AnomalLabelInfo classLabelInfo = null;
        ...
        private AnomalTrainModelInfo modelInfo = null;
        ...
        public AnomalyTrainViewModel(FileBrowserService _folderBrowserService,
                                          ...
                                          DialogService _dialogService)
        {
            ...
            this.PureAnomalyModelCollection = new ObservableCollection<DeepModelPath>();
            this.TrainSampleCollection = new ObservableCollection<AnomalTrainSample>();
        ...
        private void TrainTaskStart()
        {
            ...
            while (this.IsThreadloop == true)
            {
                ...
                try
                {
                    ...
                    var saver = tf.train.import_meta_graph(configureService.AnomalyTrainedModelUnzipPath + Path.DirectorySeparatorChar + this.modelInfo.MetaFile);
                    var sess = tf.Session(graph);

                    saver.restore(sess, configureService.AnomalyTrainedModelUnzipPath + Path.DirectorySeparatorChar + this.modelInfo.CheckFile);
                    ...
                    List<AnomalTrainSample> trainSample = this.TrainSampleCollection.ToList<AnomalTrainSample>();
                    //List<AnomalTrainSample> validationSample = new List<AnomalTrainSample>();
                    ...
                    for (int epoch = 0; epoch < this.epoch && IsInterrupt != true; epoch++)
                    {
                        ...
                        TrainingService.Shuffle<AnomalTrainSample>(trainSample);
                        for (int batchStep = 0; batchStep < this.batchStep && IsInterrupt != true; batchStep++)
                        {
                            int collectionIndex = batchStep * this.batchSize;
                            var trainImage = this.trainSampleLoaderService.LoadBatch(trainSample,
                                                                                collectionIndex,
                                                                                this.batchSize,
                                                                                isGray,
                                                                                imageWidth,
                            ...
                            sess.run(new[] { Optimizer_op }, new FeedItem(X, trainImage), new FeedItem(Phase, true), new FeedItem(LearningRateTensor, learningRate));
                            var costNDArray = sess.run(new[] { Cost }, new FeedItem(X, trainImage), new FeedItem(Phase, false));
                            var accNDArray = sess.run(new[] { Accuracy }, new FeedItem(X, trainImage), new FeedItem(Phase, false));
                        ...
                        NDArray bestScoreNDArray = null;
                        NDArray worstScoreNDArray = null;
                        ...
                        {
                            var validationImage = this.trainSampleLoaderService.LoadBatch(trainSample,
                                                    index,
                                                    1,
                                                    isGray,
                                                    imageWidth,
                            ...
                            validationImage = validationImage / 255;
                            var costNDArray = sess.run(new[] { Cost }, new FeedItem(X, validationImage), new FeedItem(Phase, false));
                            var accNDArray = sess.run(new[] { Accuracy }, new FeedItem(X, validationImage), new FeedItem(Phase, false));
                            ...
                            if (bestScore < accDoubleArray)
                            {
                                ...
                                bestScoreNDArray = classResult[0];
                                bestScoreLabelName = "Reconstruction";
                            }

                            if (worstScore > accDoubleArray)
                            {
                                worstScore = accDoubleArray;
                                worstScoreNDArray = classResult[0];
                                worstScoreLabelName = "Reconstruction";
                            }
                        ...
                        Application.Current.Dispatcher.Invoke(() =>
                        {
                            ...
                            try
                            {
                                ...
                                Mat bestMatImage = this.trainSampleLoaderService.NDArrayToMat(bestScoreNDArray * 255, imageChannel, imageHeight, imageWidth);
                                ...
                                Mat worstMatImage = this.trainSampleLoaderService.NDArrayToMat(worstScoreNDArray * 255, imageChannel, imageHeight, imageWidth);
                        ...
                        {
                            saver.save(sess, configureService.AnomalyTrainedModelUnzipPath + Path.DirectorySeparatorChar + this.modelInfo.CheckFile, write_meta_graph: false);
                            sess.close();
                            if (this.trainSampleLoaderService.ZipDeepModel(this.configureService.AnomalyTrainedModelUnzipPath,
            ...
            get
            {
                if (_TrainStartCommand == null)
                {
                    _TrainStartCommand = new RelayCommand(() =>
                    {
                        ...
                        if (this.SelectedAnomalyModel == null)
                        {
                        ...
                        this.validationSampleCount = (int)(this.TrainSampleCollection.Count);
                        this.batchStep = (int)((this.TrainSampleCollection.Count) / this.batchSize);
                        ...
                        string targetModelPath = this.SelectedAnomalyModel.FullPath;
                        string unzipPath = configureService.AnomalyTrainedModelUnzipPath;
                        ...
                        // 모델 압축 해제 
                        if (trainSampleLoaderService.UnZipDeepModel(targetModelPath, configureService.AnomalyTrainedModelUnzipPath, configureService.SecurityPassword) == false)
                        {
                        ...
                        // 모델 로드
                        this.modelInfo = trainSampleLoaderService.LoadAnomalTrainModelInfo(unzipPath + Path.DirectorySeparatorChar + configureService.ModelInfoFileName);
                        this.trainLabelOutput = this.modelInfo.MaxLabelCount;

                        // 타겟 모델 경로
                        this.preModelOutputPath = configureService.AnomalyTrainedModelContainerPath + Path.DirectorySeparatorChar + modelName;
            ...
            get
            {
                if (_OpenAugmentedLabelCommand == null)
                {
                    _OpenAugmentedLabelCommand = new RelayCommand(async () =>
                    {
                        ...
                        var result = await this.trainSampleLoaderService.LoadAnomalSamplesAsync(CurrentOpenedLabelDirectory);
        ...
        public ICommand RefreshPureModelCommand
        {
            get
            {
                if (_RefreshPureModelCommand == null)
                {
                    ...
                    {
                        this.PureAnomalyModelCollection.Clear();
                        string path = configureService.AnomalyPureModelContainerPath;
                        string[] models = Directory.GetFiles(path);
                        foreach (var model in models)
                        {
                            this.PureAnomalyModelCollection.Add(new DeepModelPath()
                            {
                                FullPath = model,
        ...
        private ObservableCollection<AnomalTrainSample> _TrainSamplesCollection = null;
        public ObservableCollection<AnomalTrainSample> TrainSampleCollection
        {
            get => _TrainSamplesCollection;
            set => Set<ObservableCollection<AnomalTrainSample>>(nameof(TrainSampleCollection), ref _TrainSamplesCollection, value);
        }


        private ObservableCollection<DeepModelPath> _PureAnomalyModelCollection = null;
        public ObservableCollection<DeepModelPath> PureAnomalyModelCollection
        {
            get => _PureAnomalyModelCollection;
            set => Set<ObservableCollection<DeepModelPath>>(nameof(PureAnomalyModelCollection), ref _PureAnomalyModelCollection, value);
        }
        ...
        private DeepModelPath _SelectedAnomalyModel = null;
        public DeepModelPath SelectedAnomalyModel
        {
            get => _SelectedAnomalyModel;
            set => Set<DeepModelPath>(nameof(SelectedAnomalyModel), ref _SelectedAnomalyModel, value);
        }
