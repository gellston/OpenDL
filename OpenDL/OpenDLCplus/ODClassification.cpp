...
odl::ODClassification::~ODClassification() {
	if (this->image_buffer == nullptr)
		free(this->image_buffer);

	if (this->labelMap == nullptr)
		free(this->labelMap);
...
odl::ODClassification::ODClassification(std::string _frzPath, std::string __xml) : imageSize(0),
																					imageWidth(0),
																					imageHeight(0),
																					isGray(false),
																					maxLabelCount(0),
	...
	if (doc.LoadFile())
	{
		...
		pRoot = doc.FirstChildElement("FreezedClassModelInfo");
		if (pRoot)
		{
			...
			this->classificationPixelSize = sizeof(float) * this->maxLabelCount;
			this->image_buffer = (float*)malloc(imageSize);
			this->labelMap = (float*)malloc(this->classificationPixelSize);
...
bool odl::ODClassification::Run(const unsigned char* _input) {
	...
	memset(this->labelMap, 0.0F, this->classificationPixelSize);
	...
	memcpy((void*)this->labelMap, data, this->classificationPixelSize);
...
int odl::ODClassification::GetLabelCount() {

	return this->maxLabelCount;
}

int odl::ODClassification::MaxScoreIndex() {
	float score = 0;
	int tempIndex = 0;

	for (int index = 0; index < this->maxLabelCount; index++) {
		if (((float*)this->labelMap)[index] > score) {
			score = ((float*)this->labelMap)[index];
			tempIndex = index;
		}
	}
...
float odl::ODClassification::MaxScore() {
	float score = 0;
	int tempIndex = 0;
...
std::vector<double> odl::ODClassification::Scores() {
	std::vector<double> list;

	for (int index = 0; index < this->maxLabelCount; index++) {
		float score = ((float*)this->labelMap)[index];
