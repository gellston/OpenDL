...
class model_classification_LightVTNet:
    ...
    def __init__(self, sess, class_count):
        ...
        with tf.variable_scope('LightVTNet'):
            self._build_net()
    ...
    def _build_net(self):
        ...
        self.X = tf.compat.v1.placeholder(tf.float32, [None, 100 * 100 * 3], name='X')
        print(self.X)
        ...
        X_input = tf.compat.v1.reshape(self.X, [-1, 100, 100, 3])

        print("=== network structure ===")

        with tf.variable_scope('conv1'):
            net = tf.compat.v1.layers.conv2d(X_input, filters=32, kernel_size=3, strides=1,  use_bias=False, kernel_initializer=tf.initializers.glorot_normal())
            net = tf.compat.v1.layers.batch_normalization(net, momentum=0.9, training=self.keep_layer)
            print("conv1=", net)

        net = inverted_bottle_neck(net, 1, 32, True, self.keep_layer, name="bottle_net1")
        ...
        with tf.variable_scope('conv2'):
            net = tf.compat.v1.layers.conv2d(net, filters=self.class_count, kernel_size=3, strides=1,  use_bias=False, kernel_initializer=tf.initializers.glorot_normal())
            net = tf.compat.v1.layers.batch_normalization(net, momentum=0.9, training=self.keep_layer)
            print("conv2=", net)

        net = tf.compat.v1.nn.avg_pool2d(value=net, ksize=[1, 5, 5, 1], strides=[1, 1, 1, 1], padding='VALID')
        ...
        self.cost = tf.compat.v1.reduce_mean(tf.compat.v1.nn.softmax_cross_entropy_with_logits_v2(logits=net, labels=self.Y), name='cost')
    ...
    def get_cost_accuracy(self, x_test, y_test, keep_prop=False):
        ...
        return self.sess.run([self.accuracy, self.cost], feed_dict={self.X: x_test, self.Y: y_test, self.keep_layer: keep_prop})

    def train(self, x_data, y_data, keep_prop=True, learn_rate=0.003):
        return self.sess.run(self.optimizer, feed_dict={self.X: x_data, self.Y: y_data, self.keep_layer: keep_prop, self.learning_rate_tensor:learn_rate})