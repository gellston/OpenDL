...
class model_anomalydetection_anogan:
    ...
    def _build_net(self):
        ...
        self.X = tf.placeholder(tf.float32, [None, 512, 512, 1], name='RealTarget')
        ...
        #self.l1_loss = tf.losses.mean_squared_error(self.X, self.G)
        self.l1_loss = tf.reduce_mean(tf.abs(self.G - self.X))
        ...
        self.accuracy = tf.reduce_mean(1 - tf.abs(self.X - self.G))
    ...
    def train_discriminator(self, x_data, keep_prop=True, learning_rate=0.003):
        return self.sess.run([self.train_D, self.loss_D], feed_dict={self.X: x_data, self.keep_layer: keep_prop, self.learning_rate_tensor:learning_rate})

    def train_generator(self, x_data, keep_prop=True, learning_rate=0.003):
        return self.sess.run([self.train_G, self.loss_G], feed_dict={self.X: x_data, self.keep_layer: keep_prop, self.learning_rate_tensor:learning_rate})
