...
class model_classification_HyTrayNetV1:

    def __init__(self, sess, class_count):
        self.sess = sess
        self.class_count = 3

        with tf.variable_scope('HyTrayNetV1'):
            self._build_net()
    ...
    def _build_net(self):
        ...
        self.X = tf.compat.v1.placeholder(tf.float32, [None, 100, 100, 1], name='X')
        print(self.X)
        ...
        with tf.variable_scope('conv1'):
            net = tf.compat.v1.layers.conv2d(self.X, filters=32, kernel_size=1, strides=1, use_bias=False,
