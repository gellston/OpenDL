...
class model_classification_LightVTNet_Gray:

    def __init__(self, sess, class_count):
        self.sess = sess
        self.class_count = class_count

        with tf.variable_scope('LightVTNet_Gray'):
            self._build_net()
    ...
    def _build_net(self):
        ...
        self.X = tf.compat.v1.placeholder(tf.float32, [None, 100 * 100], name='X')
        print(self.X)
        ...
        X_input = tf.compat.v1.reshape(self.X, [-1, 100, 100, 1])

        print("=== network structure ===")

        with tf.variable_scope('conv1'):
            net = tf.compat.v1.layers.conv2d(X_input, filters=32, kernel_size=1, strides=1, use_bias=False,
